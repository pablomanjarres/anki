...
export function LiveStats() {
  ...
  return <div className="live-stats"><div className="pocket-heading"><span>Your rhythm</span><h1>Study in motion.</h1><p>Small sessions add up.</p></div>
    {!data && <Status loading={loading} error={error} retry={reload} />}
    {data && <><div className="live-stat-grid"><div><strong>{data.reviewedToday}</strong><span>Reviewed today</span></div><div><strong>{data.reviewedThisWeek}</strong><span>This week</span></div><div><strong>{data.streak}</strong><span>Day streak</span></div>{data.retention !== undefined && <div><strong>{Math.round(data.retention <= 1 ? data.retention * 100 : data.retention)}%</strong><span>Retention</span></div>}</div>
